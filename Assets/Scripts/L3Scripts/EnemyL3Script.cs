...
public class EnemyL3Script : MonoBehaviour
{
    ...
    public GameObject UV2; //To detect player even if he is in blank situation
    public GameObject UV3; //To detect player even if he is in blank situation
    ...
    private void Update()
    {
        ...
        if (AggravateMethod()) //The Enemies notice the player
        {
            ...
            {
                enemySpeed = 0;
                target.GetComponent<PlayerL3Script>().getCaught = true;
    ...
    {
        return DistanceToPlayer() < chaseDistance || UV1.GetComponent<UVMechanism>().alarm || UV2.GetComponent<UVMechanism>().alarm || UV3.GetComponent<UVMechanism>().alarm;
    }
